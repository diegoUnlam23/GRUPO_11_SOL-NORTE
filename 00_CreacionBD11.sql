...
/*************************************************************************/
-- INSERT
/*create or alter procedure socio.insertarFactura
...
as
begin
    -- Validar que al menos uno de los dos (id_inscripcion o id_registro_pileta) esté presente
    ...
    else if @id_registro_pileta is not null
    begin
    ...
    values
        (@fecha_generacion, @fecha_vencimiento_1, @fecha_vencimiento_2, 0, @descripcion, @id_inscripcion, @id_registro_pileta, @id_estado_factura)
...
/*************************************************************************/
-- INSERT
/*create or alter procedure socio.insertarItemsFactura
    @id_factura int
as
begin
    ...
    select @id_inscripcion = id_inscripcion, @id_registro_pileta = id_registro_pileta
    from socio.factura
    where id = @id_factura

    -- Si hay inscripción asociada, procesar
    if @id_inscripcion is not null
    begin
        -- Obtener costo de la categoría de la inscripción
        ...
        -- Insertar item de categoría
        insert into socio.item_factura (id_factura, monto, tipo_item)
        values (@id_factura, @costo_categoria, 'Categoría')

        -- Sumar al total
        set @monto_total += @costo_categoria
        ...
        -- Sumar al total todas las actividades
        select @monto_total += isnull(sum(a.costo), 0)
        from socio.inscripcion_actividad ia
        join socio.actividad a on ia.id_actividad = a.id
        where ia.id_inscripcion = @id_inscripcion

        -- Insertar items por actividades extras
        insert into socio.item_factura (id_factura, monto, tipo_item)
        ...
        -- Sumar al total todas las actividades extra
        select @monto_total += isnull(sum(ae.costo), 0)
        from socio.inscripcion_actividad ia
        join socio.actividad_extra ae on ia.id_actividad_extra = ae.id
        where ia.id_inscripcion = @id_inscripcion
          and ia.id_actividad_extra is not null
    ...
    -- Si hay registro de pileta asociado, agregar costo si corresponde
    if @id_registro_pileta is not null
    begin
        declare @costo_pileta decimal(8,2) = 0

        -- Suponiendo que el registro de pileta tiene un costo
        ...
        begin
            ...
            set @monto_total += @costo_pileta
        end
    end

    -- Actualizar el monto total en la factura
    update socio.factura
    set monto = @monto_total
    where id = @id_factura

    print 'Items insertados correctamente.'
end
go*/
